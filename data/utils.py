...
def get_files(folder, name_filter=None, extension_filter=None):
    ...
    if not os.path.isdir(folder):
        raise RuntimeError("{} is not a folder.".format(folder))
...
def pil_loader(data_path, label_path):
    ...
    """
    data0 = Image.open(data_path[0])
    data1 = Image.open(data_path[1])
    data2 = Image.open(data_path[2])
    data3 = Image.open(data_path[3])
...
def pil_loader_8(data_path, label_path):
    """Loads a sample and label image given their path as PIL images.
    ...
    data2 = Image.open(data_path[2])
    data3 = Image.open(data_path[3])
    data4 = Image.open(data_path[4])
    data5 = Image.open(data_path[5])
    data6 = Image.open(data_path[6])
    data7 = Image.open(data_path[7])
    label = loadmat(label_path)['label']
    # print(type(label), label.shape)

    return [data0, data1, data2, data3, data4, data5, data6, data7], label


def pil_loader_15(data_path, label_path):
    """Loads a sample and label image given their path as PIL images.
    ...
    data6 = Image.open(data_path[6])
    data7 = Image.open(data_path[7])
    data8 = Image.open(data_path[8])
    data9 = Image.open(data_path[9])
    data10 = Image.open(data_path[10])
    data11 = Image.open(data_path[11])
    data12 = Image.open(data_path[12])
    data13 = Image.open(data_path[13])
    data14 = Image.open(data_path[14])
    # data7 = Image.open(data_path[7])
    label = loadmat(label_path)['label']
    # print(type(label), label.shape)

    return [data0, data1, data2, data3, data4, data5, data6, data7, data8,data9,data10,data11,data12,data13,data14], label
...
def enet_weighing(dataloader, num_classes, c=1.02):
    ...
    class_count = 0
    total = 0
    for _, label, _1, _2, _3 in dataloader:
...
def median_freq_balancing(dataloader, num_classes):
    ...
    for _, label in dataloader:
        ...
        total += mask * flat_label.size #flat_label.size = toral number of pixel

        # Sum up the number of pixels found for each class
        class_count += bincount

    # Compute the frequency and its median
    freq = class_count / total #freq = a list contain the frequency of a label pixel in the total pixel
    med = np.median(freq)
